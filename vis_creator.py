...
class VisSim:
    ...
    Then generate initial observed visibilities as:
        ...
        V_obs[k] = V_model[k] g[i] conj(g[j]) (1 + x[i] + conj(x[j]))
        
        OR 
        
        V_obs[k] = V_model[k] g[i] conj(g[j]) (1 + x[i]) (1 + conj(x[j]))
        
    depending on the "level" of exactness to be applied. GLS can only use
    the first form.
        
    These values are stored in the object. Any of them may be perturbed from their 
    initial values after initialization. In that case, it is possible that the equation
    above will not hold.
    ...
    def __init__(self, nant, vis_model_sigma=1000, gain_sigma=3, x_sigma=0.1, level="approx"):
        """
        Parameters
        ----------
        nant: integer
        ...
        self.x = x
        self.level = level
        self.variances = np.full(nvis, 1)      # Baseline variances

        self.V_obs = self.get_simulated_visibilities()

    def get_simulated_visibilities(self):
        """
        Calculate V_model*gains for all baselines. This may or may not be the same as V_obs.
    ...
    def get_antenna_gains(self):
        """
        Calculate the antenna gains by incorporating the gain offsets.
    ...
    def get_normalized_observed(self):
        """
        Normalized visibilities (N) are the observed visibilities minus
        the model times gains. Calculate:
        
            N[k] = V_obs[k] - V_model[k] g[i] conj(g[j])
            
        These are used in the GLS to form the equation N = P x where
        ...
        array_like
            The normalized visibilities, indexed by baseline.
        """
    ...
    def get_chi2(self):
        """
        The chi2 of the fit between V_obs and V generated by model times gains
        """
        return np.sum(np.abs(self.V_obs-self.get_simulated_visibilities())**2/self.variances)
    ...
    def print(self):
        ...
        print("Normalized observed", self.get_normalized_observed())
...
class VisTrue(VisSim):
    def __init__(self, file_root, time=0, freq=0, level="approx"):
        ...
        self.x = np.zeros(g_bar.size, dtype=np.complex64)
        self.V_obs = V
        self.variances = np.full(V.size, 1)
...
def perturb_vis(vis, vis_perturb_percent):
    ...
    if vis_perturb_percent is not None:
        ...
        if vis_perturb_percent > 0:
            ...
            V = np.where(choices <= 0, new_vis.V*(1-vis_perturb_percent/100.0), new_vis.V*(1+vis_perturb_percent/100.0))
            new_vis.V = V
...
if __name__ == "__main__":
    ...
    v.get_antenna_gains()
    v.get_normalized_observed()
    v.simulate_visibilities()
